...
ret_t pack_one_frame(uint16_t msg_class_id, uint8_t *payload, uint16_t payload_length, mac_frame_t *frame_buffer,
                     uint16_t *buffer_size)
{
    ...
    *buffer_size = sizeof(mac_frame_t) + payload_length;
    ...
    DBG("pack_one_frame success.. size:%d msg_class:%d msg_id:%d payload_length:%d\n",
        *buffer_size, frame->msg_class, frame->msg_id, frame->payload_length);
    return RET_SUCCESS;
}
...
ret_t unpack_stream_cycle(uint8_t *stream, uint16_t length) {
    ...
    DBG("into stream unpack cycle..length:%d\n", length);
    printf_buffer(stream, length);
    // TODO: 判断当length<4的情况, 仅仅当前导码存在当前流里边的情况，
    while (!is_unpack_done) {
        // 找到引导码，将后边的包全部送入buffer
        DBG("current char:0x%x ...\n", *pData);
        if (step == UNPACK_PREABLE_SYNC && *(uint32_t *)pData == PREAMBLE_SYNC_DATA) {
            pFrame = (mac_frame_t *)pData;
            DBG("new unpack buffer..payload_length:%d\n", pFrame->payload_length);
            // 清空缓冲区，开始新的解包循环
            ...
            if ((pFrame->payload_length + sizeof(mac_frame_t)) > (length - cur_read_stream_index)) {
                ...
                DBG("one frame unpacked done...cur_read_stream_index:%d\n", cur_read_stream_index);
            }
        }
        // 这个stream会包含上个没有解完的包
        ...
        DBG("char:0x%x not recognize.. go next..\n", *pData);
